...
def base(env, name, mf, assembly_time, going_main_time, components_store, transport_time):
    """
    Simula la fabricación y transporte de una placa base.
    :param env: Entorno de simulación.
    :param name: Nombre de la placa base.
    ...
    reintentos = 0
    print(f'{name}: Llegan todos los componentes de la placa base en el tiempo {round(env.now)}')
    ...
    while reintentos < max_reintentos:  # Reintentar en caso de fallo
        ...
        print(f'{name}: Fallo crítico. No se pudo fabricar la placa base.')
    ...
    with mf.request() as req:
        ...
        print(f'{name}: Empieza la fabricación de la placa base en el tiempo {round(env.now)}')
        ...
        yield components_store.put('PlacaBase')
        print(f'{name}: Placa base enviada al almacén de componentes en el tiempo {round(env.now, 2)}')