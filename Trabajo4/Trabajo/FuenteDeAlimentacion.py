...
def alimentacion(env, name, mf, assembly_time, going_main_time, components_store, transport_time):
    """
    Simula la fabricación y transporte de una fuente de alimentación.
    :param env: Entorno de simulación.
    :param name: Nombre de la fuente de alimentación.
    ...
    while reintentos < max_reintentos:  # Reintentar en caso de fallo
        ...
        print(f'{name}: Fallo crítico. No se pudo fabricar la fuente de alimentación.')
        return  # Salir de la función si se supera el límite de reintentos

    
    
    # Pedir acceso a la fábrica principal
    print(f'{name}: Llegan todos los componentes de la fuente de alimentación en el tiempo {round(env.now)}')
    with mf.request() as req:
        yield req
        
        # Simular el tiempo de ensamblaje en la fábrica principal
        print(f'{name}: Empieza la fabricación de la fuente de alimentación en el tiempo {round(env.now)}')
        ...
        yield components_store.put('FuenteDeAlimentacion')
        print(f'{name}: Fuente de alimentación enviada al almacén de componentes en el tiempo {round(env.now, 2)}')