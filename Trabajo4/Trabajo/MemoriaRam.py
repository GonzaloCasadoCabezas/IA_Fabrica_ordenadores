...
def ram(env, name, mf, assembly_time, going_main_time, components_store, transport_time):
    ...
    print(f'{name}: Llegan todos los componentes de la memoria RAM en el tiempo {env.now}')
    ...
    with mf.request() as req:
        ...
        print(f'{name}: Empieza la fabricación de la memoria RAM en el tiempo {env.now}')
        yield env.timeout(going_main_time)
        print(f'{name}: Termina el ensamblaje en el tiempo {env.now}')
